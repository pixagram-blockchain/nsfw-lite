...
# ///
"""
train_nsfw_hf_mnv4.py — same trainer as train_nsfw.py, but sourcing data from the
...
MobileNetV4-conv-small-050 backbone, the same RandomResizedCrop (train) /
plain-square-resize (val) parity transforms, the same class-weighting, loop,
and the SAME outputs — model/nsfw_mnv4.pt, model/labels.json, model/preprocess.json
...
RUN IT ON HUGGING FACE JOBS (GPU, no local setup — this file is a UV script):
    ...
    hf jobs uv run --flavor a100-large -s HF_TOKEN scripts/train_nsfw_hf_mnv4.py \
        -- --nsfw-dataset wallstoneai/civitai-top-nsfw-images-with-metadata \
           ...
           --push-repo <you>/nsfw-lite-mnv4 --epochs 12 \
...
def main():
    ...
    print("[train] next:  python scripts/export_model.py --checkpoint model/mnv4/nsfw_mnv4.pt --calib-data <calib-dir> --out-dir model/mnv4")
