...
# ///
"""
train_nsfw_hf.py — same trainer as train_nsfw.py, but sourcing data from the
...
RUN IT LOCALLY (or in a GPU Space / notebook) the ordinary way:
    ...
    python scripts/train_nsfw_hf.py \
        --nsfw-dataset wallstoneai/civitai-top-nsfw-images-with-metadata \
        --sfw-dataset  wallstoneai/civitai-top-sfw-images-with-metadata
    python scripts/train_nsfw_hf.py --data-dir ./data   # local imagefolder
...
def main():
    ...
    print("[train] next:  python scripts/export_model.py --calib-data <calib-dir>")
