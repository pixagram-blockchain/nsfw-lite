...
# ///
"""
train_nsfw_hf_tinynet.py — same trainer as train_nsfw.py, but sourcing data from the
...
TinyNet-E backbone, the same RandomResizedCrop (train) /
plain-square-resize (val) parity transforms, the same class-weighting, loop,
and the SAME outputs — model/nsfw_tinynet.pt, model/labels.json, model/preprocess.json
...
RUN IT ON HUGGING FACE JOBS (GPU, no local setup — this file is a UV script):
    ...
    hf jobs uv run --flavor a100-large -s HF_TOKEN scripts/train_nsfw_hf_tinynet.py \
        -- --nsfw-dataset wallstoneai/civitai-top-nsfw-images-with-metadata \
           ...
           --push-repo <you>/nsfw-lite-tinynet --epochs 12 \
  ...
  --push-repo so the trained model/ folder is uploaded to the Hub; then locally:
      hf download <you>/nsfw-lite-tinynet --repo-type model --local-dir model/
...
RUN IT LOCALLY (or in a GPU Space / notebook) the ordinary way:
    ...
    python scripts/train_nsfw_hf_tinynet.py \
        --nsfw-dataset wallstoneai/civitai-top-nsfw-images-with-metadata \
        --sfw-dataset  wallstoneai/civitai-top-sfw-images-with-metadata
    python scripts/train_nsfw_hf_tinynet.py --data-dir ./data   # local imagefolder
...
DEFAULT_BACKBONE = "tinynet_e.in1k"
...
def parse_args():
    ...
    p.add_argument("--backbone", default=DEFAULT_BACKBONE,
                   help="timm model id (default: tinynet_e.in1k)")
    ...
    p.add_argument("--out-dir", default="model/tinynet")
    ...
    p.add_argument("--push-repo", default=None,
                   help="repo id to upload model/ to after training (e.g. you/nsfw-lite-tinynet)")
...
def main():
    ...
    best_f1, best_path = -1.0, os.path.join(args.out_dir, "nsfw_tinynet.pt")
    no_improve = 0
    gstep = 0
    ...
    print("[train] next:  python scripts/export_model.py --checkpoint model/tinynet/nsfw_tinynet.pt --calib-data <calib-dir> --out-dir model/tinynet")
